...
export function flockOrientation(angle){const facing=Math.cos(angle)>=0?1:-1,bank=Math.max(-.16,Math.min(.16,Math.atan2(Math.sin(angle),Math.abs(Math.cos(angle)))*.35));return{facing,bank};}
