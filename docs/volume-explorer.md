...
Seven generated bird rigs (four on narrow screens) follow two joined cubic Bézier segments in a closed 60-second path. Each rig is composed from a separate body, left wing, and right wing sprite; the path controls position and heading while the wing phase rotates both wings from their attachment points. Phase offsets create cohesion and alternating lateral slots maintain separation without a simulation or new dependency. This is a lightweight guided flock, not a general-purpose boids simulation. The same path is retained in the Three.js flock helper for native validation. The overlay is z-indexed above the reader, uses transparent alpha assets, and passes all pointer input through to the controls. Local browser captures validate the cloud layers, bird parts, overlap and pointer pass-through.
