import {createFlock,FLOCK_PERIOD} from '../src/domains/atmosphere/flock.mjs';
...
const flock=createFlock(),flockScene=new THREE.Scene();flockScene.background=new THREE.Color('#d9d3b8');flockScene.add(flock.group);flock.update(0,1.5);const flockStart=render(flockScene,world.camera,{width:1200,height:800});await writeFile(path.join(out,'flock.png'),flockStart);flock.update(FLOCK_PERIOD,1.5);const flockEnd=render(flockScene,world.camera,{width:1200,height:800});if(hash(flockStart)!==hash(flockEnd))throw Error('Flock loop framebuffer does not close');
const mesa=JSON.parse(await readFile(path.resolve('../render-runtime/mesa-package.json'),'utf8'));
const report={passed:true,flockLoopFrameMatches:true,flockSha256:hash(flockStart),renderer:'@headless-three/renderer 0.4.3 — native WGPU/Vulkan',three:THREE.REVISION,adapter:'Mesa Lavapipe',icd:process.env.VK_ICD_FILENAMES,package:mesa.Version,packageSha256:mesa.SHA256,dimensions:[1200,800],bytes:first.length,deterministic:hash(first)===hash(second),sha256:hash(first),animatedFrameSha256:hash(after),productionScene:'src/domains/atmosphere/world.mjs',systemFilesModified:false};await writeFile(path.join(out,'validation.json'),JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));world.dispose();
